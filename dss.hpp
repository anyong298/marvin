        ...
        for (int i=0;i<len;++i){
          ...
          if (box_reg&&(phase==Training||is_render)){
            ...
            if (hasTarget>0){
              ...
              fread((void*)(box_tar_diff.diff), sizeof(float), 6, fp);
              if (box_2dreg&&phase==Training){
                  fread((void*)(box_tar_diff.diff2d), sizeof(float), 4, fp);
                  float diff2d_full[4];
                  fread((void*)(diff2d_full), sizeof(float), 4, fp);
              }
              if (orein_cls&&phase==Training){
                  fread((void*)(&box_tar_diff.oreintation), sizeof(int), 1, fp);
  ...
  */
  ...
  void prefetch(){
    ...
    for (int batch_id =0; batch_id < sum(batch_size); ++batch_id){
      ...
      fread(anchor_Idx, sizeof(uint8_t), len, fp);

      uint8_t* x_Idx = new uint8_t[len];
      fread(x_Idx, sizeof(uint8_t), len, fp);

      uint8_t* y_Idx = new uint8_t[len];
      fread(y_Idx, sizeof(uint8_t), len, fp);

      uint8_t* z_Idx = new uint8_t[len];
      fread(z_Idx, sizeof(uint8_t), len, fp);
      
      float* oscf = new float[len];
      fread(oscf, sizeof(float), len, fp);
      
      float* diff = new float[6*len];
      fread(diff, sizeof(float), 6*len, fp);
...
public:
    ...
    void init(){
      ...
      if (len>0) fread((void*)(height_dis_file.data()), sizeof(char), len, fp);
      std::ifstream infile; 
      ...
      while (feof(fp)==0) {
        ...
        if (len>0) fread((void*)(scene->mesh_file.data()), sizeof(char), len, fp);
  ...
  void init(){
    ...
    while (feof(fp)==0) {
      ...
      for (int bid = 0;bid<len;bid++){
         Box2D box;
         fread((void*)(&(box.category)), sizeof(unsigned int),   1, fp);
         fread((void*)(box.tblr),        sizeof(float), 4, fp);
